...
void SparkBuildApp::ShowMainMenu() {
    ...
    switch (choice) {
        case 1: ShowEnvironmentMenu(); break;
        case 2: ShowConfigureMenu(); break;
        case 3: ShowBuildMenu(); break;
        case 4: ShowConfigSummary(); break;
        case 0: m_running = false; break;
        default:
...
void SparkBuildApp::ShowEnvironmentMenu() {
    ...
    while (inMenu) {
        ...
        switch (choice) {
            ...
            case 2: SetEnginePath(); break;
            case 3: CloneEngine(); break;
            case 4: DownloadCMake(); break;
            case 5: InitSubmodules(); break;
...
void SparkBuildApp::CheckEngineRepo() {
    ...
    if (!enginePath.empty() &&
        std::filesystem::exists(std::filesystem::path(enginePath) / "CMakeLists.txt")) {
...
void SparkBuildApp::CheckSubmodules() {
    ...
    if (total == 0) {
        ...
        Term::PrintStatus("Git Submodules", false,
                          std::to_string(missing) + "/" + std::to_string(total) + " missing - run 'Init Submodules'");
    }
}
...
void SparkBuildApp::DownloadCMake() {
    ...
    std::string cmakeBin = (std::filesystem::path(destDir) / "cmake-3.31.5-windows-x86_64" / "bin" / "cmake.exe").string();
#elif defined(SPARK_PLATFORM_MACOS)
    std::string url = "https://github.com/Kitware/CMake/releases/download/v3.31.5/cmake-3.31.5-macos-universal.tar.gz";
    std::string cmakeBin = (std::filesystem::path(destDir) / "cmake-3.31.5-macos-universal" / "CMake.app" / "Contents" / "bin" / "cmake").string();
#else
    std::string url = "https://github.com/Kitware/CMake/releases/download/v3.31.5/cmake-3.31.5-linux-x86_64.tar.gz";
    std::string cmakeBin = (std::filesystem::path(destDir) / "cmake-3.31.5-linux-x86_64" / "bin" / "cmake").string();
#endif
...
void SparkBuildApp::ShowConfigureMenu() {
    ...
    while (inMenu) {
        ...
        switch (choice) {
            case 1: ShowGeneratorMenu(); break;
            case 2: ShowBuildTypeMenu(); break;
            case 3: SetEnginePath(); break;
            case 4: SetBuildPath(); break;
            case 5: ShowOptionsMenu(); break;
            case 6: ShowPresetsMenu(); break;
            case 7: ShowCMakePresetsMenu(); break;
#ifdef SPARK_PLATFORM_WINDOWS
            case 8: {
                std::string ts = Term::ReadLine("MSVC Toolset (e.g. v143, v144): ");
                if (!ts.empty()) m_config.config.msvcToolset = ts;
                break;
            }
#endif
            case 9: {
                ...
                if (!js.empty()) {
                    try { m_config.config.parallelJobs = std::stoi(js); } catch (...) {}
                }
                break;
            }
            case 0: inMenu = false; break;
...
void SparkBuildApp::ShowGeneratorMenu() {
    ...
    for (int i = 0; i < (int)gens.size(); i++) {
        ...
        std::cout << "  " << Term::Bold(std::to_string(i + 1)) << "  "
                  << GeneratorDisplayName(gens[i]) << Term::Green(marker) << "\n";
    }
    std::cout << "\n";
...
void SparkBuildApp::ShowBuildTypeMenu() {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        std::cout << "  " << Term::Bold(std::to_string(i + 1)) << "  "
                  << BuildTypeToString(types[i]) << Term::Green(marker) << "\n";
    }
    std::cout << "\n";
    ...
    while (inMenu) {
        ...
        for (auto cat : catOrder) {
            ...
            for (int i = 0; i < (int)m_config.config.options.size(); i++) {
                ...
                if (idx < 10) num = " " + num;
                std::cout << "   " << Term::Bold(num) << "  "
                          << state << " " << opt.displayName
...
void SparkBuildApp::ShowBuildMenu() {
    ...
    while (inMenu) {
        ...
        switch (choice) {
            case 1: GenerateProject(); break;
            case 2: BuildProject(); break;
            case 3: GenerateAndBuild(); break;
            case 4: CleanBuild(); break;
            case 5: OpenBuildFolder(); break;
            case 6: RunEngine(); break;
            case 0: inMenu = false; break;
        }
    }
}
...
void SparkBuildApp::RunEngine() {
    ...
    try {
        for (const auto& entry : std::filesystem::recursive_directory_iterator(binDir)) {
            ...
            if ((perms & std::filesystem::perms::owner_exec) == std::filesystem::perms::none) continue;
#endif
...
void SparkBuildApp::ShowConfigSummary() {
    ...
    std::cout << "  " << Term::Bold("Engine Path:  ") << (m_config.config.enginePath.empty() ? "(not set)" : m_config.config.enginePath) << "\n";
    std::cout << "  " << Term::Bold("Build Path:   ") << m_config.config.buildPath << "\n";
    std::cout << "  " << Term::Bold("CMake Path:   ") << (m_config.config.cmakePath.empty() ? "(PATH)" : m_config.config.cmakePath) << "\n";
...
#ifdef SPARK_PLATFORM_WINDOWS
    std::cout << "  " << Term::Bold("MSVC Toolset: ") << (m_config.config.msvcToolset.empty() ? "(default)" : m_config.config.msvcToolset) << "\n";
#endif
    std::cout << "  " << Term::Bold("Parallel:     ") << (m_config.config.parallelJobs == 0 ? "auto" : std::to_string(m_config.config.parallelJobs)) << "\n";
    if (!m_config.config.cmakePreset.empty()) {
